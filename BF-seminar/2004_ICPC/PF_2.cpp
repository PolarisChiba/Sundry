...
int main() {
	...
	while (true) {
		...
		vector<vector<pair<int, pair<int, int>>>> cmpScore(2 * images.size(), vector<pair<int, pair<int, int>>>(2 * images.size()));
		for (int i = 0; i < (int)images.size(); ++ i) {
			for (int j = i + 1; j < (int)images.size(); ++ j) {
				cmpScore[i][j] = Compare(images[i], images[j]);
			}
		}
		...
		while (true) {
			...
			for (int i = 0; i < (int)images.size(); ++ i) {
				...
				for (int j = i + 1; j < (int)images.size(); ++ j) {
					auto [score, offset] = cmpScore[id[i] - 1][id[j] - 1];
			...
			for (int i = 0; i < (int)images.size() - 1; ++ i) {
				cmpScore[id[i] - 1][id.back() - 1] = Compare(images[i], images.back());
			}
